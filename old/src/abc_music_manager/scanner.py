...
from .abc_parser import parse_abc_file, ParsedSong
from .db.folder_rule import get_enabled_roots
from .db.song_repo import ensure_song_from_parsed, logical_identity, find_song_by_logical_identity
...
def run_scan(
    ...
    *,
    ...
    if not all_roots:
        # No roots: remove all indexed files and orphan songs so library is empty
    ...
    # Remove SongFile rows for paths no longer found (folder removed or files deleted)
...
def _remove_missing_song_files(conn: sqlite3.Connection, current_paths: set[str]) -> None:
    ...
    if not current_paths:
        # No roots or no files: remove all SongFiles (and orphan Songs)
        conn.execute("DELETE FROM SongFile")
    else:
        # Delete SongFiles whose path is not in the current scan set.
        # Use a temp table to avoid SQLite bound-parameter limit (e.g. 999) for large libraries.
    ...
    # Orphan songs: no SongFile left pointing to them. Delete dependents first (FK), then Song.
    conn.execute(
